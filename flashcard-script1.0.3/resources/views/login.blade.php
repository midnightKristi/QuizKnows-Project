...
<html>
  <head>
    ...
    <title>{{ Config::get('SITE_TITLE') }} | {!! trans('flashcard.logIn') !!}</title>
  ...
  <div class="wrapper">
    ...
    <div class="login-box">
      ...
      <div class="login-box-body">
        <p class="login-box-msg">{!! trans('flashcard.signIn') !!}</p>
        <div class="divide-line"></div>
        @include('common.errors')
        <form action="{{ Config::get('RELATIVE_URL') }}/login" method="post">
        	{{ csrf_field() }}
          <div class="form-group has-feedback">
            <input type="text" class="form-control" placeholder="{!! trans('flashcard.username') !!}" name="username" value="{{ old('username') }}">
          </div>
          <div class="form-group has-feedback">
            <input type="password" class="form-control" placeholder="{!! trans('flashcard.password') !!}" name="password">
          </div>
          <div class="row">
            <div class="col-xs-8">
              <div class="checkbox icheck">
                <label>
                  <input type="checkbox" name="rememberMe" onclick="this.value=this.checked"> {!! trans('flashcard.rememberMe') !!}
                </label>
              </div>
            </div><!-- /.col -->
            <div class="col-xs-4">
              <button type="submit" class="btn btn-success btn-block btn-flat btn-lg">{!! trans('flashcard.signIn') !!}</button>
            </div><!-- /.col -->
          </div>
        </form>
    ...
    <div style="padding-top:30px;">
    	<center><a href="{{ Config::get('RELATIVE_URL') }}/forgot-password" style="color:#414141;font-style: italic;">{!! trans('flashcard.forgotYourPassword') !!}</a><br></center>
    </div>    
    </div><!-- /.login-box -->
